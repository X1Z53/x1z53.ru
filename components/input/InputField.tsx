import { Flex, Input, InputGroup, InputLeftAddon, InputRightAddon, NumberDecrementStepper, NumberIncrementStepper, NumberInput, NumberInputField, NumberInputStepper, Select, Textarea } from "@chakra-ui/react"
...
export default function InputField(props: InputFieldProps) {
  const { type, title, value, onChange, inGroup, readOnly, colorPickerButton, copyButton, clearButton, isFirst, isLast, styles } = props
  ...
  return <InputGroup {...styles} flexDirection={["column", "column", "row"]} >
    ...
    <Flex width="100%">
      ...
      {
        type === "group" && children
          .map((child, index) => cloneElement(child, { inGroup: true, isFirst: !index, isLast: index === children.length - 1 }))
      }
      {
        ...
        >
          ...
          {colorPickerButton && <ColorPickerButton {...{ value, onChange }} styles={rightStyles} />}
          {clearButton && <ClearButton {...{ useClear }} styles={rightStyles} />}
        </InputRightAddon>
      }
    </Flex>
  </InputGroup >
