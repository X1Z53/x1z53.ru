    ...
    case GameActionTypes.FoundationAddCards:
      return produce(state, draft => {
        ...
        if (action.payload.movedCard.state === CardState.Deck) {
          ...
          draft.stacks[action.payload.movedCard.index] = draft.stacks[action.payload.movedCard.index].filter(
    ...
    case GameActionTypes.StackAddCards:
      return produce(state, draft => {
        if (action.payload.movedCard.state === CardState.Stack) {
          const indexOfCard = draft.stacks[action.payload.movedCard.index].findIndex(
            card => card.id === action.payload.movedCard.id
          )
          const cardsToAdd = draft.stacks[action.payload.movedCard.index]
            .slice(indexOfCard)
            .map(card => ({ ...card, index: action.payload.index, isDragging: false }))
          const cardsLeft = draft.stacks[action.payload.movedCard.index].slice(0, indexOfCard)
          draft.stacks[action.payload.index] = [...draft.stacks[action.payload.index], ...cardsToAdd]
    ...
    case GameActionTypes.TurnCard:
      return produce(state, draft => {
        draft.stacks[action.payload.index].map((card, i) => {
          if (card.id === action.payload.id && i === draft.stacks[action.payload.index].length - 1) {
            card.hidden = false
            return card
          }
          return card
        ...
        switch (action.payload.movedCard.state) {
          ...
          case CardState.Stack:
            draft.stacks[action.payload.movedCard.index] = draft.stacks[
              ...
              index >= draft.stacks[action.payload.movedCard.index].findIndex(
